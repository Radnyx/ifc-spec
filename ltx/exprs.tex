...
\begin{Enumeration}{MonadicOperator}
	...
	\enumerator{MsvcIsImplicitLifetime}
...
\ifcSortSection{New}{MonadicOperator} 
Source-level ``\code{new}'' operator.

When used as the value of the \field{assoc} field of a \sortref{Monad}{ExprSort} structure, 
that structure reprsents a \grammar{new-expression} of the form \code{new T}, 
and the \field{argument} of that structure is an abstract reference designating the type \code{T}.

\note{See also \sortref{LookupGlobally}{MonadicOperator} for the representation of a \grammar{new-expression} of the form \code{::new T}}

\ifcSortSection{Delete}{MonadicOperator}
Source-level ``\code{delete}'' operator.

When used as the value of the \field{assoc} field of a \sortref{Monad}{ExprSort} structure, 
that structure reprsents a \grammar{delete-expression} of the form \code{delete p}, 
and the \field{argument} of that structure is an abstract reference designating the expression \code{p}.

\note{See also \sortref{LookupGlobally}{MonadicOperator} for the representation of a \grammar{delete-expression} of the form \code{::delete p}}

\ifcSortSection{DeleteArray}{MonadicOperator}
Source-level ``\code{delete[]}'' operator.
...
\ifcSortSection{MsvcIsImplicitLifetime}{MonadicOperator}
Source-level ``\code{__builtin_is_implicit_lifetime}'' operator (\sortref{MsvcHasUserDestructor}{SourceKeyword}).
...
Abstraction machine operation corresponding to default construction of an object at a given address.

When used as the value of the \field{assoc} field of a \sortref{Dyadic}{ExprSort} structure, 
that structure reprsents a placement \grammar{new-expression} of the form \code{new(p) T}, 
and the \field{argument[0]} of that structure is an abstract reference designating the expression \code{p} 
(which can be an \grammar{expression-list}) and the \field{argument[1]} is an abstract reference designating the type \code{T}.

\note{See also \sortref{LookupGlobally}{MonadicOperator} for the representation 
of a placement \grammar{new-expression} of the form \code{::new(p) T}}
...
Abstract machine operation corresponding to allocating appropriate storage and default-initializing an array of a given element type and length. 

When used as the value of the \field{assoc} field of a \sortref{Dyadic}{ExprSort} structure, 
that structure represents a \grammar{new-expression} of the form \code{new T[n]}, 
and the \field{argument[0]} of that structure is an abstract reference designating the type \code{T} 
and the \field{argument[1]} is an abstract reference designating the length \code{n}.
...
represents a placement \grammar{new-expression} of the form ``\code{new(p) T(x)}'' or ``\code{new(p) T{x}}'' 
 where
\begin{itemize}
	\item the first operand is the placement expression, which can be an \grammar{expression-list} 
	\item the second operand designates the constructed type
	\item the third operand is the initializer
\end{itemize}
...
\begin{itemize}
	...
	\item the third operand designates the initializer
\end{itemize}
...
Source-level ``\code{operator new}'' \grammar{operator-function-id}.

\ifcSortSection{AllocateArray}{StorageOperator}
Source-level ``\code{operator new[]}'' \grammar{operator-function-id}.

\ifcSortSection{DeallocateSingle}{StorageOperator}
Source-level ``\code{operator delete}'' \grammar{operator-function-id}.
